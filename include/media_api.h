 ...
 * 返回值	:0表示成功负值表示失败
 **********************************************************************************************/
int moveto_media_packet(IO media_source_t *media,IN int place);
 ...
 * 返回值	:正值表示存入buf中的有效字节数,负值表示出错
 **********************************************************************************************/
static __inline__ int read_media_packet(IN media_source_t *media,OUT void *buf,IN int buf_len,OUT int *eleseq,OUT int *flag)
{
	return MShmPoolGetPacket(&media->mpool,buf,buf_len,eleseq,flag);
}
...
/**********************************************************************************************
 * 函数名	:read_media_frame_type()
 * 功能	:返回缓冲区内帧的类型
 * 输入	:meida:描述已连接的媒体缓冲池结构指针
 * 返回值	:缓冲区内的帧类型
 **********************************************************************************************/

static __inline__  int read_media_frame_type(IN media_source_t * media)
{

	return GetFrameFlag(&media->mpool);
