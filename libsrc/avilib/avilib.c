...
#include<devinfo.h>		//// 2009-9-23 lsk TEST real D1
#endif
...
static inline int avi_add_entry_file(FILE *fp,unsigned char *tag, long flags, long pos, long len)
{
	char tmp[16];
	memcpy(tmp,tag,4);
	long2str(&tmp[4],flags);
	long2str(&tmp[8],pos);
	long2str(&tmp[12],len);	
	fwrite((void*)tmp,1,16,fp);	
	return 16;
}
static int avi_add_index_entry(avi_t *AVI, unsigned char *tag, long flags, long pos, long len)
{
	...
	else
	{
		...
		//将索引写到索引文件中
		avi_add_entry_file(AVI->fp_idx,tag,  flags,  pos,  len);
	}
...
#ifndef _WIN32	
	//shixin added
	p=strrchr(filename,'/');//查找/
	if(p==NULL)
	{
		p=(char*)filename;//单独的文件
	}
	else
	{//包含目录
		p++;
...
#if EMBEDED			//Added by shixin 
	sprintf(AVI->idx_file,"/tmp/%s.widx",p);
#else
	sprintf(AVI->idx_file,"%s.widx",filename);
#endif

   AVI->fp_idx=fopen(AVI->idx_file,"w");		//如果不能打开则还是实用内存存放索引
#else
	AVI->fp_idx=NULL;
#endif
   return AVI;
...
//将索引文件的内容写入avi文件的尾部
//返回写入的字节数,负值表示出错
static int AVI_make_index(avi_t *AVI,char *idx_name)
{
	FILE *rd_fp=NULL;
	int    bytes=0;
	int	ret;
	...
	while(1)
	{
		ret=fread((void*)buf,1,sizeof(buf),rd_fp);
		if(ret>0)
		{
			bytes+=ret;
			avi_write(AVI->fdes,buf,ret);
		}
		else
		{
			break;
...
/*
函数名	:AVI_fix_header
参数	:AVI:指向一个已经打开并设置好音视频参数的avi_t结构的指针
		 writeindex:为1则写入索引,否则不写入索引
功能	:向它相关的avi文件写入文件头和索引
返回值	:0表示正常,负值表示有错误发生
by wsy@ Dec,2006
 */
int AVI_fix_header(avi_t *AVI,int writeindex)
{
   int ret, njunk, sampsize, hasIndex, ms_per_frame, frate, idxerror, flag,type;
...
//	init_devinfo();		//读取设备信息，lsk 2009 -9-23
   ...
   /* Try to ouput the index entries. This may fail e.g. if no space
	...
	if(writeindex==1)
	{
	   ...
	   else
	   {
	   	fclose(AVI->fp_idx);
		AVI->fp_idx=NULL;
	      	memcpy(AVI_header,"idx1",4);
   	      	long2str(&AVI_header[4],AVI->n_idx*16);
	   	ret=avi_write(AVI->fdes,AVI_header,8);
		AVI->pos += 8;	//update pos
		ret=AVI_make_index(AVI,AVI->idx_file);
		if(ret>0)
		{
			AVI->pos +=ret;
			ret=0;
			remove(AVI->idx_file);//删除idx文件
		}
		else
			ret=-1;
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   // changed by lc
   OUTLONG(AVI->a_rate*(sampsize/4)/AVI->a_chans/AVI->a_chans);//changed by shixin 1000*AVI->mp3rate/8);

   OUTLONG(0);             /* Start */
   OUTLONG(4*AVI->audio_bytes/sampsize);   /* Length */
//// lsk 2009-9-23
//  OUTLONG(0);             /* SuggestedBufferSize */
   OUTLONG(1024*2*AVI->a_chans);             /* SuggestedBufferSize */
   OUTLONG(-1);            /* Quality */
   ...
   // changed by lc
   OUTLONG(AVI->a_rate / AVI->a_bits * 8 / AVI->a_chans );          /* SamplesPerSec */
   // ThOe
   OUTLONG(AVI->a_rate*(sampsize/4)/AVI->a_chans);//changed by shixin 1000*AVI->mp3rate/8);
   //ThOe (/4)
...
//// lsk 2009-9-23		for real D1
#ifdef  REAL_D1_TEST_FOR_3000
	type=get_devtype();
	if((type>=T_GTVS3021)&&(type<T_GTVM3001))
	{
		if((AVI->width==720)&&(AVI->height==576))
		{
			AVI_header[nhb]=0x01;  //// 0x01 是真D1的标志
		}
		if(njunk > strlen(id_str)+12) 
		{
			sprintf(id_str,"AVI file");
			memcpy(AVI_header+nhb+4, id_str, strlen(id_str));
		}
	}
	else
	{
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   // ThOe /4
   OUTLONG(sampsize/4);      /* Scale */
   OUTLONG(AVI->a_rate*(sampsize/4));//changed by shixin 1000*AVI->mp3rate/8);
   OUTLONG(0);             /* Start */
   OUTLONG(4*AVI->audio_bytes/sampsize);   /* Length */
   OUTLONG(0);             /* SuggestedBufferSize */
   OUTLONG(-1);            /* Quality */
...
static int gen_index_from_file(char *filename,const char *indexname,int *v_frames,int *a_frames)
{
	const static int key_frame_flag=0xb0010000;
	int *flag=NULL;
	avi_t *ravi = NULL;
	FILE *fp_idx=NULL;
	int videos=0;
	...
	int keyflag=0;
	int n;
	if((filename==NULL)||(indexname==NULL)||(v_frames==NULL)||(a_frames==NULL))
		return -1;
	ravi=AVI_open_input_file(filename,0);	
	if(ravi==NULL)
	{
		printf("AVI_open_input_file open:%s=NULL!!\n",filename);
		return -1;
	}	
	fp_idx=fopen(indexname,"w");
	if(fp_idx==NULL)
	{		
		printf("can't create idx file:%s!!\n",indexname);
		AVI_close(ravi);
		return -1;
	}	
        offset=lseek(ravi->fdes,HEADERBYTES,SEEK_SET);//shixin fixed
	while(1)
	{
	       
		if( avi_read(ravi->fdes,data,8) != 8 )
		{//文件结束
			break;
		}
		...
		{
			 lseek(ravi->fdes,4,SEEK_CUR);
			 continue;
		}

     		n = PAD_EVEN(str2ulong(data+4));
		offset=lseek(ravi->fdes,0,SEEK_CUR);
		if(offset<8)
			break;
		offset-=8;	//写入索引的数据偏移量 因为前面读过8字节了，所以要减8
	      if(strncasecmp(data,ravi->video_tag,3) == 0)
	      {
	      		videos++;
			if( avi_read(ravi->fdes,data,4) != 4 )
			{//文件结束
				break;
			}	
			flag=(int*)data;
			if(*flag==key_frame_flag)
			{
				keyflag=1;
				//printf("*");
			}
			else
			{
				...
				//printf(".");
	      		...
	      		avi_add_entry_file(fp_idx,(unsigned char *)"00dc",keyflag, offset, n);	   
			n-=4;//因为之前读过4字节了
	            	if(lseek(ravi->fdes,n,SEEK_CUR)<0)
				break;
	      }
      		else if(strncasecmp(data,ravi->audio_tag,4) == 0)
             {//音频
             		audios++;
			avi_add_entry_file(fp_idx,(unsigned char *)"01wb",0, offset, n);	  
             		if(lseek(ravi->fdes,n,SEEK_CUR)<0)
				break;
             }
		else  if(lseek(ravi->fdes,n,SEEK_CUR)<0)  
			break;
...
//返回该文件的秒数，负值表示出错
//同一个文件shixinadded
int avi_fix_bad_file(char *filename)
{
	int ret;
	char *p=NULL;
	int videos;
	int audios;
	//char tmp[256];
	avi_t *AVI = NULL;		//输入文件的avi_t结构指针
	if(filename==NULL)
		return -1;
	
//	ret=gen_index_from_file(filename,idx,&tmp,&tmp);
...
//改为写入方式
	...
	ret=gen_index_from_file(filename,AVI->idx_file,&videos,&audios);
	AVI->pos=lseek(AVI->fdes,0,SEEK_END);
	if((AVI->pos&1)==1)
		AVI->pos=lseek(AVI->fdes,1,SEEK_END);
	AVI->n_idx=ret;
	AVI->fp_idx=fopen(AVI->idx_file,"r");
	if(AVI_frame_rate(AVI)!=0)
	{
		ret=videos/AVI_frame_rate(AVI);
	}
	else
		ret=0;
	//strncpy(tmp,AVI->idx_file,sizeof(tmp));
	 AVI_close(AVI);
	// remove(tmp);//删除idx文件
	 //删除idx文件
	 return ret;
...
static int avi_write_data(avi_t *AVI, char *data, long length, int audio, int keyframe)
{
   ...
   if(audio)
      ...
      n = avi_add_index_entry(AVI,"00dc"/*"00db"*/,((keyframe)?0x10:0x0),AVI->pos,length);
   ...
   if(audio)
      ...
      n = avi_add_chunk(AVI,"00dc"/*"00db"*/,data,length);
...
int AVI_dup_frame(avi_t *AVI)
{
   ...
   if(avi_add_index_entry(AVI,"00dc"/*"00db"*/,0x10,AVI->last_pos,AVI->last_len)) return -1;
   AVI->video_frames++;
...
avi_t *AVI_open_input_file(char *filename, int getIndex)
{
  ...
  if(AVI_parse_input_file(AVI, getIndex)<0)	//changed by shixin 
  	return 0;
...
int AVI_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  /* Read first 12 bytes and check that this is an AVI file */
   ...
   //if(!AVI->movi_start) ERR_EXIT(AVI_ERR_NO_MOVI)	//remed by shixin 
