...
#ifndef AVILIB_H
#define AVILIB_H
//zw-del 2011-09-19 #define REAL_D1_TEST_FOR_3000
...
{
   int	fdes;              /* File descriptor of AVI file *///changed by shixin from int
   long   mode;              /* 0 for reading, 1 for writing */
   char  idx_file[200];	//索引文件名
   FILE 	*fp_idx;		//index_file,name=file.ridx(read mode) name=file.widx(write mode) //shixin added
...
void AVI_set_video(avi_t *AVI, int width, int height, double fps, char *compressor);//"Divx" for compressor
...
参数	:AVI:指向一个已经打开并设置好音视频参数的avi_t结构的指针
		 writeindex:为1则写入索引,否则不写入索引(创建文件时不写,关闭文件时写)
