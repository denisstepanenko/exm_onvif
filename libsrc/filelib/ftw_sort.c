...
{	
	...
	char work_dir[1024*3];
	char temp_work_dir[1024*3];	//文件名长度限制在1024个字符内，可具体更改
	//char *mem_dir;				//每次递归调用内的目录记录
	char mem_dir[1024*3];			//每次递归调用内的目录记录
	...
	if ((lstat(dir, &statbuf)) != 0) 
	{
		...
		if(fn!=NULL)
		{
			fn_ret_val = fn(dir,&statbuf,FTW_NS);
			//printf("fn--%d\n",__LINE__);
			if(fn_ret_val !=0)
			{
	...
	if (S_ISDIR(statbuf.st_mode)) 
	{
		...
		if(fn!=NULL)
		{
			fn_ret_val = fn(dir,&statbuf,FTW_D);
			//printf("fn--%d\n",__LINE__);
			if(fn_ret_val !=0)
			{
		...
		{	
			...
			if(fn!=NULL)
			{
				fn_ret_val = fn(dir,&statbuf,FTW_DNR);	
				//printf("fn--%d\n",__LINE__);
				if(fn_ret_val !=0)
				{	
		...
		{			
			...
			//lint -e534
			check_dir_val = check_dir(	work_dir,
							fn,
							dir_depth,
							sort_mode,
							rm_empty_dir,
	...
	else
	{
		...
		if(fn!=NULL)
		{
			fn_ret_val = fn(dir,&statbuf,FTW_F);
			//printf("fn--%d\n",__LINE__);
			if(fn_ret_val !=0)
			{
				dir_info->retu_val = fn_ret_val;
...
*/
	...
	//调用遍历函数进行遍历
	ftw_no_chdir_val = check_dir(	dir,
							fn,
							dir_depth,
							sort_mode,
							rm_empty_dir,
