...
static __inline__ void SemAddAll(MSHM_POOL *pool)
{
	...
	for(i=0;i<SHPOOL_MAX_USERS;i++)
	{
		if(usr->valid)
		{
			...
			//printf("Semgetval[%d]=====[%d]\n",i,SemVal);
			if(SemVal>ph->maxsem)
			{
				//printf("%s user:%d(%s) maybe exited(%d) ,delete it!\n",mc->name,i,usr->name,SemVal);
			...
			if(rc<0)
			{
				...
				if(rc==-ERANGE)
				{
					printf("%s user:%d maybe exited ,delete it!\n",mc->name,i);
					usr->valid=0;//用户可能已经不存在了,删除用户
...
获取帧类型
*/
int GetFrameFlag(MSHM_POOL *pool)
{
	SHPOOL_HEAD	*ph=pool->ph;	
	MEM_CORE		*mc=pool->mc;
	SHPOOL_ELE		*ele;
...
/**************************************************************************
  ...
  *************************************************************************/
int MShmPoolGetPacket(MSHM_POOL *pool,void *buf,int buflen,int *eleseq,int *flag)
{
	SHPOOL_HEAD	*ph;
	MEM_CORE		*mc;
	SHPOOL_USR		*usr;
	...
	while((ph->tail==ph->head)||(cur==ph->tail))
	{		
...
/**************************************************************************
  ...
  *参数	: pool:指向缓冲池的指针 
  			  place:要移动的数量
  			  	...
  			  	0表示不用移动
  *返回值	:返回>=0表示移动的单元数, 负值表示失败错误是负的errno
  *	注意:只比上面的函数少了places=moved;
  *************************************************************************/
int MShmPoolMvNewPlace(MSHM_POOL *pool,int place)
{
	int i;
	int num=0;	//
	int temp;
