...
const int audio_fmt=1000;//给出一个极限值，目的是从头连接
/**********************************************************************************************
	 * 函数名	:init_audio_enc()
 * 功能	:初始化音频编码器缓冲池结构
 * 输入	:无
...
int init_audio_enc(void)
{
	...
	for(i=0;i<total;i++)
	{
		ret=init_media_rw(&audio_enc[i],MEDIA_TYPE_AUDIO,i,AAC_AUDIO_BUFLEN);
		ret=init_media_rw(&audio_enc_playback[i],MEDIA_TYPE_AUDIO,i,AAC_AUDIO_BUFLEN);
 ...
 * 注		:在程序启动时调用一次
 **********************************************************************************************/
int init_audio_dec_usr(int no)
{
	

	int i;
    int ret;
	media_source_t *adec=&audio_dec_usr[no];
	
	for(i=0;i<MAX_AUDIO_DECODER;i++)
	{
	
		init_media_rw(adec,MEDIA_TYPE_AUDIO,i,1024);
		adec++;
	}
...
int init_audio_dec_pool(void)
{
	...
	{
		init_media_rw(adec,MEDIA_TYPE_AUDIO,i,1024);
		ret=create_media_write(adec,get_audio_dec_key(i),"rtimage",128*1024);
		adec++;
	}
    //if(ret>=0)
    //{
...
int connect_audio_enc(IN int no,IN int type,IN char *name,IN int pre_sec)
{
	...
	if(no>=get_audio_num())
		return -EINVAL;
	ret=connect_media_read(&audio_enc[no], get_onvif_pool_key(no,type)+0x20000,name,MSHMPOOL_NET_USR);
	if(ret>=0)
	{
...
int connect_audio_enc_playback(IN int no,IN int type,IN char *name,IN int pre_sec)
{
	...
	ret=connect_media_read(&audio_enc_playback[no], get_onvif_pool_key(no,type)+0x20000,name,MSHMPOOL_NET_USR);
	if(ret>=0)
	{
		stat=get_aenc_stat(no);
		if(stat==ENC_STAT_OK)
		{//编码器状态正常
			...
			if(attrib!=NULL)
			{
				...
				audio=&media->a_fmt;
				frate=audio->a_frate;
				printf("audio frate [%d]\n",frate);
				//rc=move_media_place(&audio_enc_playback[no],-(audio_fmt*pre_sec));
				rc=move_media_place(&audio_enc_playback[no],-(frate*pre_sec));
			}
		}
	}
	gtloginfo("用户[%s]连接到第[%d]缓冲池 key[%#x]\n",debug_name,no,get_audio_enc_key(no));
...
int get_frate_from_aenc(int no)
{
	int ret;
	media_attrib_t *attrib=NULL;
	audio_format_t *audio=NULL;
...
int reactive_audio_enc_playback(IN int no)
{
	if(no>=1)//fixme 当有多路音频芯片时
		return -EINVAL;	
	return reactive_media_usr(&audio_enc_playback[no]);
}
...
int init_audio_rec_enc(int no)
{
	...
	if(no > MAX_AUDIO_ENCODER)
	{
		syslog(LOG_ERR,"get_audio_num=%d MAX_AUDIO_ENCODER=%d!!!\n",no,MAX_AUDIO_ENCODER);
	}
	
	ret = init_media_rw(&audio_enc_rec[no],MEDIA_TYPE_AUDIO,no,AAC_AUDIO_BUFLEN);
