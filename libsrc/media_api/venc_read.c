...
static media_source_t video_enc_record[MAX_RECORD_CHANNEL];
...
int connect_video_enc(IN int no,IN int type,IN char *name,IN int pre_sec)
{
	...
	printf("oooooooo %#x\n",get_onvif_pool_key(no,type));
	ret=connect_media_read(&video_enc[no], get_onvif_pool_key(no,type),name,MSHMPOOL_NET_USR);
...
int connect_video_enc_succ(int no,int type,char *name,int pre_sec)
{
    ...
    while(1)
    {
    	ret=connect_video_enc(no,type,name,pre_sec);
    	if(ret==0)
    	{
 ...
 * 返回值	:0表示成功，负值表示失败
 **********************************************************************************************/
int connect_video_enc_playback(IN int no,IN int type,IN char *name,IN int pre_sec)
{
	int ret;
	int rc;
	int stat;
	...
	ret=connect_media_read(&video_enc_playback[no], get_onvif_pool_key(no,type),name,MSHMPOOL_NET_USR);
	if(ret>=0)
	{
		stat=get_venc_stat(no);
		if(stat==ENC_STAT_OK)
...
int get_frate_from_venc(int no)
{
	int ret;
	media_attrib_t *attrib=NULL;
	video_format_t *video=NULL;
...
int find_playback_iframe(IN int no)
{
	int num=0;
	if(no>=get_videoenc_num())
		return -EINVAL;	
	...
	while(read_media_frame_type(&video_enc_playback[no])!=0)
	{
		printf("frame_type=%d\n",read_media_frame_type(&video_enc_playback[no]));
		move_media_place(&video_enc_playback[no],1);
		num++;
	}
	return num;
}
int recover_playback_iframe2normal(IN int no,IN int count)
{
	if(no>=get_videoenc_num())
		return -EINVAL;	
	
	move_media_place(&video_enc_playback[no],-count);
	printf("recover_playback_iframe2normal[%d][%d]\n",no,-count);
	gtloginfo("recover_playback_iframe2normal[%d][%d]\n",no,-count);
	return 0;
}
...
int reactive_video_enc_playback(IN int no)
{
	if(no>=get_videoenc_num())
		return -EINVAL;	
	return reactive_media_usr(&video_enc_playback[no]);
}
...
/**********************************************************************************************
 * 函数名   :init_video_record_enc(int video_channel)
...
int init_video_record_enc(int videoenc_channel)
{
    ...
    if(videoenc_channel > MAX_RECORD_CHANNEL)
    {
        syslog(LOG_ERR,"hdmodule init  %d video get_videoenc_num=%d MAX_VIDEO_ENCODER=%d!!!\n",videoenc_channel,get_videoenc_num(),MAX_VIDEO_ENCODER);
        printf("hdmodule init  %d video get_videoenc_num=%d MAX_VIDEO_ENCODER=%d!!!\n",videoenc_channel,get_videoenc_num(),MAX_VIDEO_ENCODER);
    }

    ret = init_media_rw(&video_enc_record[videoenc_channel],MEDIA_TYPE_VIDEO,videoenc_channel,MAX_FRAME_SIZE);
...
{
    media_source_t  *media = &video_enc_record[no];

    if(no >= MAX_RECORD_CHANNEL)
        return NULL;
...
void *get_video_record_buf(IN int no)
{
    ...
    return video_enc_record[no].temp_buf;
...
int get_video_record_buf_len(IN int no)
{
    ...
    return video_enc_record[no].buflen;
...
int reactive_video_record_enc(IN int no)
{
    ...
    return reactive_media_usr(&video_enc_record[no]);
...
int get_video_record_enc_remain(IN int no)
{
    ...
    return read_media_remain(&video_enc_record[no]);
 ...
 *                  -EINVAL表示参数非法
 **********************************************************************************************/
int get_video_record_key(int no)
{
    switch(no)
    {
        case 0:
            return get_video_enc_key(5);
        break;
        default:
            return -EINVAL;
        break;
 ...
 * 返回值   :0表示成功，负值表示失败
 **********************************************************************************************/
int connect_video_record_enc(IN int no,IN char *name,IN int pre_sec)
{
    int ret;
    int rc;
    int stat;
    ...
    ret=connect_media_read(&video_enc_record[no], get_video_record_key(no),name,MSHMPOOL_NET_USR);
    if(ret>=0)
    {
        stat=get_record_venc_stat(no);
        if(stat==ENC_STAT_OK)
        {//编码器状态正常
            attrib=get_venc_attrib_record(no);
            if(attrib!=NULL)
            {
                video=&attrib->fmt.v_fmt;
                frate=video->v_frate;
                rc=moveto_media_packet(&video_enc_record[no],-(frate*pre_sec));                
            }
        }
    }
    return ret;
...
int get_record_venc_stat(IN int no)
{
...
int connect_video_record_enc_succ(int no,char *name,int pre_sec)
{
   ...
   if(no >= MAX_RECORD_CHANNEL)
    ...
    while(1)
    {
        ret=connect_video_record_enc(no,name,pre_sec);
        if(ret==0)
        {
            printf     ("connect videoenc%d success\n",no);
              gtloginfo("connect videoenc%d success\n",no);
...
{
	return read_media_packet(&video_enc_record[no],frame,buf_len,eleseq,flag);
}
...
int disconnect_video_record_enc(IN int no)
{
   ...
   if(no >= MAX_RECORD_CHANNEL)
	return -EINVAL;	
   return disconnect_media_read(&video_enc_record[no]);
}
