...
static media_source_t video_enc_record[MAX_RECORD_CHANNEL_M];
...
/**********************************************************************************************
 * 函数名   :init_video_record_enc_all(l)
...
int init_video_record_enc_all()
{
	...
	{
            ret += init_media_rw(&video_enc_record[i],MEDIA_TYPE_VIDEO,i,MAX_FRAME_SIZE);
	}
...
int disconnect_video_record_enc(IN int no)
{
   ...
   if((video_enc_record[no].dev_stat==-1)||(video_enc_record[no].attrib=NULL)||(video_enc_record[no].mpool.mc ==NULL))
   {
      return -EINVAL;
   }
