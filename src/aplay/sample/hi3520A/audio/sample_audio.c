...
#define APLAY_AUDIO_DEV     2  //音频设备编号
#define APLAY_AUDIO_DEC     1  //音频解码器编号
#define APLAY_AUDIO_DEV_CHN 0 //音频设备通道号
...
static tcprtimg_svr_t     *p_tcprtimg=NULL;      
...
******************************************************************************/
static FILE *SAMPLE_AUDIO_OpenAdecFile(ADEC_CHN AdChn, PAYLOAD_TYPE_E enType,char * filename)
{
    FILE *pfd;
    //HI_CHAR aszFileName[128];
...
******************************************************************************/
HI_S32 SAMPLE_AUDIO_AdecAo(AIO_ATTR_S *pstAioAttr,char * filename)
{
    HI_S32      s32Ret;
    AUDIO_DEV   AoDev = APLAY_AUDIO_DEV;

    AO_CHN      AoChn = APLAY_AUDIO_DEV_CHN;
    ADEC_CHN    AdChn = APLAY_AUDIO_DEC;
    ...
    pfd = SAMPLE_AUDIO_OpenAdecFile(AdChn, gs_enPayloadType,filename);
    if (!pfd)
    {
...
static int attach_tcprtimage_para(void)
{
    ...
    mc=MShmCoreAttach(0x41000,(void**)&p_tcprtimg);
    if(mc==NULL)
        return -ENOMEM;
    return 0;
}
static void close_all_res(void)
{
	int maxopen,i;
...
static void HandleSig(int signo)
{
		...
		if(SIGTERM==signo)    
		{
			SAMPLE_COMM_AUDIO_DestoryTrdFileAdec(1);
...
HI_S32 main(int argc, char *argv[])
{
	...
	attach_tcprtimage_para(); //连接rtimage共享内存
	tcprtimg_svr_t *p=p_tcprtimg;
	av_server_t     *av_svr=&p->av_server;
    aplay_server_t *ap_svr=&p->aplay_server;
	pthread_mutex_lock(&ap_svr->s_mutex);
	if(ap_svr->usrs==1)
	{
		printf("rtimage正在使用下行音频，故退出\n");
		pthread_mutex_unlock(&ap_svr->s_mutex);
		exit(0);
    ...
    if (argc >= 2)
    {
		...
		if(argv[2]==NULL)
		{
			printf("Usage: aplay [decode type] [filename]\n");
			return HI_FAILURE;
    	}
    }
	else
    ...
    if (HI_SUCCESS != s32Ret)
    {
	 ...
	 s32Ret = SAMPLE_AUDIO_AdecAo(&stAioAttr, argv[2]);
