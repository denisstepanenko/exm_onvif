...
int hd_write(int fd, long long  seek, unsigned int blk_num, char *write_buf,unsigned int buf_size)
{
 ...
 * 返回值：读的字节数， -1失败
 ********************************************************************************/
int hd_read(int fd, long long  seek, unsigned int blk_num, char *read_buf,unsigned int buf_size)
{
	int dio_tmp=0;
    int blks_read=0;
    int blk_sz=BLOCKSIZE;
