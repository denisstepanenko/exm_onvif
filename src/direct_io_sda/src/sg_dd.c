...
main(int argc, char * argv[])
{
    ...
    wrkPos=(char *)malloc(blocks*blk_sz);
    if(wrkPos==NULL)
    {
    	printf("malloc error \n");
    	close(outfd);
    ...
    int i;
    //for(i=0; i<blocks*blk_sz; i++)
    //	wrkPos[i]=0xfe;
    memset(wrkPos, 0xff, blocks*blk_sz);
    ret=sg_write(outfd, wrkPos, blocks, seek, blk_sz, &oflag, &dio_tmp);
    printf("ret=%d\n",ret);
    printf("dio_tmp:%d\n",dio_tmp);
#else
    inf[0] = '\0';
    ...
    while (dd_count > 0) {
        ...
        if (sparse_skip) {
            ...
            first = 1;
            while (1) {
            	memset(wrkPos, 0xff, blocks*blk_sz);//yk add
