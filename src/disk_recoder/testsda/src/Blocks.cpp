...
Blocks::Blocks(Blocks *bs,unsigned int uBufSize)
{
	this->cBuff = bs->cBuff;
	this->uNumOfBlocks = get_block_num(uBufSize);
	this->uSize = uBufSize;
	this->bType = bs->bType;
	this->llSeek = bs->llSeek;
cout << "uNumOfBlocks: " << uNumOfBlocks << " uSize: " << uSize << " seek: " << llSeek << endl;
	bs->cBuff = NULL;
}
Blocks::~Blocks()
{
