...
#define AUDIO_ENC_POOL_SIZE_NET 24*1024
#define VIDEO_POOL_SIZE 1024*1024
#define BUFFER_SIZE  400*1024					//帧缓冲大小
...
typedef struct{
	int						playback_a_cnt;		///报警之后编码器持续录像的时间单位s
...
public:
	...
	unsigned int  get_frame_buff_size(){return buffsize;};
	unsigned int  get_remain_frame_num();
...
class MediaPoolWrite
{
public:
	MediaPoolWrite(int format);
	int write_pool(char *buf, unsigned int buf_size);
private:
	int format;
	char name[256];
	media_source_t media;
