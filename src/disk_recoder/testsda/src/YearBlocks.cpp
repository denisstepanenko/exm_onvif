...
int YearBlocks::add(const struct  seek_block &sb)
{
	...
	gtloginfo("year blocks,add,tail:%d,size:%d",yeardata->year_queue_data.queue_tail,yeardata->year_queue_data.queue_size);
	//写入数据
	if(write() < 0)
	{
		gtloginfo( "hd_write error\n");
		return HD_ERR_WRITE;
	}
	return 0;
}
...
int YearBlocks::del(const struct  seek_block &sb)
{
	...
	{
		gtloginfo("yearblocks emputy\n") ;
		return BLOCK_ERR_EMPTY;//空了
	}
	gtloginfo("yearblocks del day:%d, seek:%lld\n",sb.time,sb.seek);
	...
	gtloginfo("yearblocks head:%d,size:%d",yeardata->year_queue_data.queue_head,yeardata->year_queue_data.queue_size);
	return 0;
}
