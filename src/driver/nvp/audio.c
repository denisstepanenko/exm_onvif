...
void audio_init(unsigned char dec, unsigned char ch_num, unsigned char samplerate, unsigned char bits)
{
	...
	//gpio_i2c_write(dec, 0x14, 0x00); //zsk add	
...
void set_audio_parm(int dec,vdec_audio_volume_parm * parm)
{	
	parm->AIN_1=(parm->AIN_1<<4)|parm->AIN_0;
	parm->AIN_3=(parm->AIN_3<<4)|parm->AIN_2;
	printk("AIN_1=%#X\n",parm->AIN_1);
	printk("AIN_3=%#X\n",parm->AIN_3);
        gpio_i2c_write(dec,0xff,0x01);
        gpio_i2c_write(dec,0x01,parm->AIN_1);
        gpio_i2c_write(dec,0x02,parm->AIN_3);
	parm->AOUT=parm->AOUT<<4|0;
	printk("AOUT=%#X\n",parm->AOUT);
        gpio_i2c_write(dec,0x22,parm->AOUT);
