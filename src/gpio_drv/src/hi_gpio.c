...
#define  GPIO_0_BASE_ADDR  0x20150000   //0x2015_0000
...
static int gpio_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GPIO_READ_BIT:
	    ...
	    group_bit_info.value = reg_tmp;
            copy_to_user((void __user *)arg, &group_bit_info, sizeof(gpio_groupbit_info));

            break; 
	case GPIO_WRITE_BIT:
	    copy_from_user(&group_bit_info,(gpio_groupb

		it_info*)arg, sizeof(gpio_groupbit_info));
