...
static const char	prog_pkt[]      = "/conf/gt1000.tar.gz";			///< 打包好的设备程序包文件名
static const char	backup_pkt[]  = "/hqdata/hda1/firmware/gt1000.tar.gz";	///< 备份的程序文件包
...
int main(int argc,char *argv[])
{
	...
	sprintf( cmdbuf,"tar zxvf %s -C %s >/tmp/extract.txt 2>>/tmp/extract.txt ", prog_pkt, target_dir );
	printf( "prog_cmd = %s \n", cmdbuf );
	...
	if( ret!= 0 )
	{
		...
		sprintf( cmdbuf,"tar zxvf %s -C %s >/tmp/extract.txt 2>>/tmp/extract.txt ", backup_pkt, target_dir );
		printf( "backup_cmd = %s \n", cmdbuf );
