...
#define  VERSION	"0.54"
/*
 * 0.54     沿用三型机机制，但修改为适用于IP1004设备的情况
...
void insert_mac(void)
{
        ...
        {
                errno=0;
				//lc to do 重新加载驱动，换为新的网卡模块
				//ret=system("lsmod | grep \"ftmac100\"");
                //tmp_errno=errno;
                ret=system("lsmod | grep \"hidmac\"");
                tmp_errno=errno;
                printf("查找网卡驱动的结果[%d]\n",ret);
                if(tmp_errno==0)
                {
                        ...
                        else
                        {
								...
								system("/sbin/insmod /usr/lib/ko_hi3520A/hidmac.ko"); 
								//system("/sbin/insmod /usr/lib/ko_hi3520A/hidmac.ko use_fiq=0");
                        }
                }
                else
                {
...
static int process_ifstat_change(char *dev,int stat,int flag)
{
	...
	if(stat==1)
	{
		...
		if(strncmp(dev,"eth0",5)==0)
		{
			...
			//lc to do 在ip1004中，暂时没有网卡驱动问题，不需要重新加载驱动
/*
			if(strncasecmp(typestr,"GTIP1",5)==0&&(flag==1))
			{
				...
				ret=system("rmmod hidmac");
				sleep(1);
				ret=system("/sbin/insmod /usr/lib/ko_hi3520A/hidmac.ko"); 
#if 0
				////// use_fiq=0"
#endif				
				insert_mac();
...
*/	
			...
			*/
			...
			{
				ret=system("killall -15 dhcpcd");
				ret=system("/usr/sbin/dhcpcd eth0 &");
