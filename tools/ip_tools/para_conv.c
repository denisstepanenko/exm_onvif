...
//lcdo ppp0 配置文件变为/etc/ppp/pppoe.conf 
#define PPP0_FILE "/etc/ppp/pppoe.conf"
//#define PPP0_FILE 	"/etc/sysconfig/network-scripts/ifcfg-ppp0"
//lc do 目前ip1004不需要vsftp功能，简单的日志下载和查看通过http服务实现
...
//2.2  解决boa.conf中出现乱码的问题
//2.1  将工具移植到GTIP设备中，功能不变，部分路径改完
/*
...
int print_helpmsg(void)
{
	...
	printf("\t-h\t输出帮助信息\n");
	printf("\t-v\t输出版本信息\n");
	printf("\t-m\t多转一，即 多个配置文件 -> ip1004.ini\n");
	printf("\t-s\t一转多，即   ip1004.ini -> 其他多个配置文件\n\n");
	return(0);
}
...
int set_boa_str(FILE *fp,char *key,char *value)
{
	...
	rewind(fp);
	//lc do 2013-6-20 修改boa配置导致多处奇怪字符
	memset(tmp,0,15000);
...
int chap_to_ini(dictionary *ini,char *filename, char *sec1, char *sec2,char *def)
{
	...
	if(strcmp(iniusr,chapusr)!=0)
		{
			printf("ip1004.ini的%s按chap由%s修改为%s\n",sec1,iniusr,chapusr);
			gtloginfo("%-30s%s->%s\n",sec1,iniusr,chapusr);
			change_flag=1;
			iniparser_setstr(ini,sec1,chapusr);
		}
	if(strcmp(inipswd,chappswd)!=0)
		{
			printf("ip1004.ini的%s按chap由****修改为****\n",sec2);
			gtloginfo("%-30s****->****\n",sec2);
			change_flag=1;
			iniparser_setstr(ini,sec2,chappswd);
		}
...
int boa_to_ini(dictionary *ini, char *secini, char *secboa, char *def)
{
	...
	if((strcmp(iniport,boaport)!=0))
		{
			printf("ip1004.ini的%s按boa由%s修改为%s\n",secini,iniport,boaport);
			gtloginfo("%-30s%s->%s\n",secini,iniport,boaport);
			iniparser_setstr(ini,secini,boaport);
			change_flag=1;
		}
 ...
 *		-h:输出帮助信息；
 *		-v:输出转换工具的版本信息；
 *		-m:表示多转一，即 多个配置文件->ip1004.ini；
 *		-s:表示一转多，即ip1004.ini->其他多个配置文件；*/
	init_devinfo();		 
